# 🇲🇹 Malta — kompletny plan 29.09–06.10 (7 nocy)

Przylot **29.09 (wt), 12:05** · Wylot **06.10 (wt), 12:30** (na lotnisku do **10:15**). Baza: **St Julian's (Argento)**. Transport: **autobusy Tallinja + promy** — bez taksówek. **7-dniowy Explore (25 €)** pokrywa wszystkie autobusy D1–D8. Plik zawiera: hotel, bilety, plan godzinowy, transport, oceny atrakcji, budżet, praktyczne, rady blogów i **rezerwacje online z linkami**.

> **Spis treści:**
> [1. Hotel](#hotel) ·
> [2. Bilety](#bilety) ·
> [3. Elastyczność dni](#elastycznosc) ·
> [4. Plan godzinowy D1–D8](#plan) ·
> [5. Oceny atrakcji](#oceny) ·
> [6. Budżet](#budzet) ·
> [7. Praktyczne](#praktyczne) ·
> [8. Rady blogów](#blogi) ·
> [9. Rezerwacje online](#rezerwacje)
>

## 1. Gdzie nocować — przegląd 4 opcji i rekomendacja

| Hotel | Cena | Lokalizacja | Śniadanie | Ocena / najważniejsze |
| --- | --- | --- | --- | --- |
| **Argento Hotel** ⭐4 | 3 918 zł | **St Julian's, Spinola Bay** — cicha dzielnica, ~2 min do przystanku, 10 min do plaży St George's | Tak | 🏆 **Rekomendacja.** Cicha dzielnica Spinola, blisko plaży i przystanku — dobra baza na D3 (bus 13/14/15 → Valletta, skąd szybki prom na Gozo) i D5 (222 → Ċirkewwa na Comino). 4.3/5 |
| **Orbis Boutique Studios by Holi** ⭐4 | 2 758 zł | **Qormi** — centralna, lądowa dzielnica, 3 km od lotniska, 2 min od przystanku (61/62 do Valletty) | Nie (aneks) | 💡 **Najlepszy budżetowy.** 8.7/10, self-catering (kuchnia + pralka), możesz gotować. Minusy: brak morza/plaży, bez gwarantowanego śniadania, dalszy dojazd na D3 (Valletta) i D5 (Ċirkewwa) |
| **ddream Hotel** ⭐3 | 3 645 zł | St Julian's, **środek Paceville** (nocne życie) | Tak | ✖️ **Słaba wartość** — ~300 m od Argento (lokalizacja praktycznie ta sama), a gorszy (środek Paceville = hałas nocny, skargi o remont/łazienki) |
| **THE OSiRiS** (Marsaxlokk) | 2 621 zł | **Marsaxlokk** — wioska rybacka, płd.-wsch. | Nie | ✖️ Zła baza na objazdówkę: do Valletty/Mdiny/Gozo 1–1,5 h autobusem dziennie |

> 🏆 **Argento (3 918 zł)** — najlepszy wybór w St Julian's: cicha dzielnica Spinola, dobra ocena, blisko plaży i przystanku. ⚠️ **Ważne:** Argento i ddream są **~300 m od siebie** (lokalizacja niemal identyczna), a na Gozo w D3 jedzie się autobusem 13/14/15 (~25 min) do Valletty, skąd odchodzi szybki prom (Gozo Highspeed, Grand Harbour) — więc żaden z tych hoteli nie jest znacząco bliżej i argument „przystań promów" ich **nie różnicuje**. Wybór to jakość/cisza vs cena: Argento (cichy, +273 zł) vs ddream (głośny Paceville, gorsze opinie, −273 zł) → rekomendacja **Argento**. Dodatkowo Argento = dobra baza na D3 (bus do Valletty na Gozo) i D5 (Comino przez Ċirkewwę) oraz śniadania i restauracje w zasięgu pieszym.
>
> 🏆 💡 **Orbis (2 758 zł)** — jeśli chcesz **zaoszczędzić ~1160 zł**: świetna cena i ocena, ale to **aparthotel/self-catering** (kuchnia + pralka, **bez gwarantowanego śniadania hotelowego**), brak morza, mniej restauracji/życia wieczornego, a D3 (bus do Valletty na Gozo) i D5 (Comino przez Ċirkewwę) wymagają dodatkowego dojazdu. Sens ma, gdy oszczędność na noclegu jest kluczowa i realnie skorzystacie z kuchni.
>
> 🏆 ✖️ **ddream i Osiris** — odradzam (słaba wartość / zła lokalizacja).
>

## 2. Bilety — 7-dniowy Explore (25 €)

**✅ Zalecenie: 7-dniowy bilet Explore (25 €).** Jeśli w D2 i D4 (Valletta, Three Cities) jedziesz **autobusem 13/14/15 zamiast promu Sliema→Valletta**, te przejazdy też pokrywa karta — wtedy 7-dniówka wychodzi taniej i prościej. ⚠️ **Explore nie obejmuje linii „Tallinja Direct" (TD)** — one kosztują 3 €/przejazd dodatkowo. Wszystkie trasy w tym planie (poza opcjonalnym TD2 na lotnisko) używają zwykłych linii objętych kartą.

> 7-dniowa liczy od pierwszego użycia (D1 ~13:00) i pokrywa **cały pobyt**, także poranny dojazd 13/14/15+88 w D8 (wygasa ~06.10 13:00). **Oszczędzasz ~6,5 €.** Uwaga: na lotnisko najprościej TD2, ale to **+3 € i poza kartą** — dlatego w planie trasa 88 + 13/14/15 (w karcie).
>

**Osobno płacisz (nie wchodzi w kartę):**

- Szybki prom Valletta→Gozo (D3) — **~7,50 € w jedną (~15 € w obie)**, Gozo Highspeed (kup bilet online); alternatywa przez Sliema ~8,50 €/jedna strona; budżetowo prom Gozo Channel 4,65 €
- Blue Grotto rejs (D6) — **~10 €** (ok. 20–25 min; kupno na miejscu, bufor do 12)
- Comino prom Ċirkewwa→Comino (D5) — **~15 € return/os** (online ~14 €)
- Prom Valletta→Three Cities (D4) — **~3 € one way / ~5 € return** (taryfa dzienna; nocna 3,50/5,20)
- Wejścia do atrakcji

### Układ dni

| Dzień | Środek | Karta 7 dni? |
| --- | --- | --- |
| D1 wt 29.09 | 88 → Valletta → 13/14/15 (lotnisko) | ✅ |
| D2 śr 30.09 | Valletta — **autobus 13/14/15** | ✅ |
| D3 czw 01.10 | **Gozo** (bus 13/14/15 do Valletty + szybki prom Valletta→Gozo + lokalne + prom osobno) | ✅ |
| D4 pt 02.10 | Three Cities — **autobus 13/14/15** (+prom do Birgu osobno) | ✅ |
| D5 sob 03.10 | **Comino — 222 + prom z Ċirkewwy** | ✅ (222 na karcie; prom osobno) |
| D6 nd 04.10 | **Blue Grotto (rano) + Marsaxlokk (obiad)** (13/74/81/119/201) | ✅ |
| D7 pon 05.10 | **Mdina** (202) | ✅ |
| D8 wt 06.10 | **13/14/15 → 88 na lotnisko** | ✅ |

> 💡 Prom Sliema→Valletta (D2) możesz wybrać dla widoku i szybkości (~10 min vs ~25 min autobusem), ale płacisz ~2 €/rejs osobno. Dla maksymalnej oszczędności — autobus.
>

## 3. Elastyczność dni — co sztywne, co zalecane, co dowolne

| Dzień | Sztywne? | Uzasadnienie |
| --- | --- | --- |
| D1 wt · przylot | 🔒 twardo | lot przylotowy 12:05 — nie do zmiany |
| D8 wt · wylot | 🔒 twardo | lot wylotowy 12:30 — nie do zmiany |
| D6 nd · Blue Grotto + Marsaxlokk | 🔵 dowolne | targ rybny i tak zniknie do południa (Blue Grotto rano), więc **niedziela nie jest już kotwicą** — dzień łączony ze względu na długą trasę |
| D7 pon · Mdina + Rabat | 🔒 zalecane pon | **Muzeum Katedralne w Mdina zamknięte w niedziele**, otwarte w pon. Mdiny nie rób w niedzielę |
| D3 czw · Gozo | 🟡 zalecane dzień powszedni | promy szybkie codziennie, ale w weekend na Gozo tłoczniej; czwartek OK |
| D2 śr · Valletta | 🟡 zalecane wcześnie | St John's „na świeżo"; salut 12:00 codziennie. W zasadzie dowolne |
| D5 sob · Comino | 🔵 dowolne | Blue Lagoon rano — każdy dzień OK; weekend nieco tłoczniejszy (Notte Bianca wieczorem) |
| D4 pt · Three Cities | 🔵 dowolne | można przesuwać bez konsekwencji |

> Jedyne twarde punkty to loty (D1/D8) i Mdina w pon (D7, unikaj niedzieli). D6 nie jest już zależne od niedzieli. D4/D5 można dowolnie przestawiać — np. przy złej pogodzie na Comino zamienisz D5 z D4 lub D7 bez łamania zasad. Nie rób dwóch wysp (Gozo+Comino) ani dwóch miast (Valletta+Three Cities) z rzędu — na zmianę miasto/wyspa.
>

## 4. Plan dzień po dniu — godzinowo

Legenda: ⭐ warte · ◽ opcjonalne · ⚠️ sprawdź/średnio warte. **🕐 = gdzie być wcześnie, by uniknąć tłumów.**

### Dzień 1 · wt 29.09 — Przylot + St Julian's 88 + 13/14/15

| Godz. | Co |
| --- | --- |
| 12:05 | Lądowanie Luqa |
| 12:45–13:00 | Bagaż + **kup 7-dniowy Explore (25 €)** w kiosku Tallinja (hala przylotów). Na lotnisko/ze lotniska jeżdżą: **88** (zwykła, w cenie karty) i **TD2** ("Direct", **+3 €, poza kartą**) |
| 13:15 | **Autobus 88 → Valletta** (karta) → przesiadka **13/14/15 → St Julian's** (karta). **Pierwsze przyłożenie karty = na 88.** *(Wygodniej: TD2 prosto do St Julian's, ale +3 € i bez karty.)* |
| 14:30–15:00 | Hotel Argento (Spinola Bay), zostaw bagaż |
| 15:30–17:00 | ⭐ Spacer **Spinola Bay** + **Portomaso Marina** (bezpłatnie) |
| 17:00–18:30 | ⭐ **St George's Bay** na zachód słońca (10 min pieszo) |
| 19:00 | Kolacja St Julian's |

> Wartość: darmowe — ⭐ warte. Brak tłumów tego dnia.
>

### Dzień 2 · śr 30.09 — Valletta (pełny dzień) 13/14/15 · karta

| Godz. | Co |
| --- | --- |
| 08:35 | Do przystanku **Ferries (Sliema)** (~10 min pieszo) |
| 08:45 | **Autobus 13/14/15 → Valletta** (~20–25 min) — **karta** *(opcjonalnie prom ~10 min za ~2 €)* |
| 09:00 | ⭐ **St John's Co-Cathedral** (15 €) — **🕐 bądź na otwarciu 09:00, przed grupami** |
| 09:00–10:45 | Wnętrze + Caravaggio + audio-guide (w cenie) |
| 11:00–11:45 | Spacer **Republic Street**; ◽ **Grandmaster's Palace** 12 € — ⚠️ **Sala Tronowa zamknięta do odwołania; State Rooms tylko w wybrane dni od 13:30 (nie w nasze)** → pomiń |
| 11:45 | **🕐 dojdź do Upper Barrakka przed 12:00** |
| 12:00 | ⭐ **Salut armatni** (Upper Barrakka) — bezpłatnie |
| 12:30–13:30 | Lunch |
| 14:00–15:00 | ⭐ **Lower Barrakka** + mury — bezpłatnie |
| 15:00–16:00 | ◽ **MUŻA** 8 € ⚠️ · **Archeologia** 10 € ⚠️ · **Malta Experience** 16 € 🚫; **max 1** |
| 16:00 | (opcjonalnie 2. salut armatni) |
| 17:00–17:30 | **Autobus 13/14/15 Valletta → Sliema** — karta *(opcjonalnie prom)* |

> 🎉 ⚠️ **Alternatywa, jeśli pominiesz Grandmaster's Palace / muzea:** kawa w kawiarni przy Upper Barrakka + spacer po bastionach i Lower Barrakka — najlepszy „darmowy" zamiennik; ewentualnie **Casa Rocca Piccola** (10 €, pałac + schrony bombowe).
>

### Dzień 3 · czw 01.10 — Gozo (pełny dzień) bus → Valletta + szybki prom · karta 🔴

> 🎉 **🚤 Dojazd na Gozo — domyślnie szybki prom z Valletty (decyzja):** autobus 13/14/15 do Valletty (~25 min) → **terminal Lascaris Wharf** (pod murami, ~40 m poniżej dworca — zejście **windą Barrakka 1 €** / darmowym **X133** z dworca, stanowisko A1 / ~10 min pieszo w dół) → **bez długiej jazdy 222 do Ċirkewwy** (Comino w D5 zostaje przez Ċirkewwę).
>
> 🎉 **(domyślne) Valletta → Gozo:** Gozo Highspeed **~45 min bezpośrednio** do Mġarr, **~7,50 € w jedną (~15 € w obie)**; wybierz opcję „Return", promocja zmienna do ~12 €); całoroczny; kup bilet online. https://gozohighspeed.com
>
> 🎉 **(alternatywa) Sliema → Gozo (przez Buġibba):** Gozo Highspeed ~1 h 15 min (krótka przesiadka — nie jeden bezpośredni rejs), **~8,50 € w jedną (~17 € w obie)**, sezon marzec–październik — wygodniej, bo powrót kończy się w St Julian's, a ostatni kurs Mġarr→Sliema jest później (21:15→22:30). ⚠️ Terminal ze Sliemy jest na The Strand (~25–35 min pieszo od Argento / krótki bus), nie przy samym hotelu.
>
> 🎉 **(budżetowo) autobus 222 + prom Gozo Channel:** ~1 h 45–2 h 15 min, **4,65 € w obie** (bus na karcie) — taniej, ale dłużej i jedziesz do Ċirkewwy.
>

| Godz. | Co |
| --- | --- |
| 07:25 | Wyjście z hotelu → przystanek **Ferries (Sliema)** (~10 min pieszo) |
| 07:45 | **Autobus 13/14/15** → **Valletta** (~25 min, karta) |
| 08:10 | Dworzec Valletta (na górze) → **zejście na terminal Lascaris Wharf**: **winda Barrakka 1 €** (z Upper Barrakka) **lub darmowy X133** (stanowisko A1, zsynchronizowany z promami) **lub ~10 min pieszo w dół** → terminal ~08:25 |
| 08:45 | 🚤 **Szybki prom Valletta → Mġarr** (Gozo Highspeed, **~45 min bezpośrednio**; **~7,50 € w jedną (~15 € w obie)**, kup wcześniej). Terminal Lascaris Wharf — pod murami, nie przy dworcu |
| 09:30 | Z Mġarr: **301 → Victoria** (~12 min, co ~30 min) — karta |
| 10:00–11:30 | ⭐ **Victoria / Citadella** — bezpłatnie; panorama |
| 11:30–12:30 | Lunch Victoria |
| 12:45 | **307 → Xaghra** (~15 min) — karta |
| 13:00–14:30 | ⭐ **Świątynie Ġgantija / Ta' Kola combo** (10 €, UNESCO — starsze niż piramidy). 🕐 po południu spokojniej. otwarte od **~09:00** (godziny letnie HM do 31.10; ostatnie wejście 30 min przed zamknięciem) |
| 14:30–15:15 | ◽ **1 opcja tylko**: **Ta' Pinu** (308 z Victorii, bezpłatnie) **lub** **Tal-Mixta/Ramla Bay** (302/310/322). Nie pakuj więcej niż 1 — potem czeka prom |
| 15:15–16:00 | **307 Xagħra → Victoria** (~13 min) + **301 Victoria → Mġarr** (~12 min) — ~30–40 min z przesiadką; na prom 16:45 wyjedź z Xagħry najpóźniej 15:23 |
| 16:45 | 🚤 **Szybki prom Mġarr → Valletta** (~45 min, ~7,50 €) → Valletta ~17:30 → **13/14/15 → Sliema** (~25 min, karta) → ~18:00. Alternatywa: powrót przez **Sliema** (Mġarr→Sliema **17:15 → ~18:30**, osobny bilet ~8,50 €). Odejścia Mġarr→Valletta co ~godzinę (ost. **20:45 → 21:30**) |
| 18:00 | Kolacja St Julian's — wieczór wolny |

> 🕐 Wyjdź z hotelu **07:20–07:30, nie później** (bus ~07:45 → prom 08:45), by mieć cały dzień na Gozo — boarding zamyka się **2 min przed odjazdem**, a Lascaris Wharf szuka się pierwszy raz dłużej. **Miej monetę 1 € na windę Barrakka (D3)** — w D4 winda jest w cenie return promu. Realny czas hotel → pokład to ~60–70 min (bus + zejście windą/X133). Liczba przejazdów: ~5 (bez długiej 222). **Lżejszy logistycznie niż wersja przez Ċirkewwę** i szybszy niż Sliema (bez przesiadki w Buġibbie).
>

### Dzień 4 · pt 02.10 — Three Cities autobus + prom do Birgu osobno

| Godz. | Co |
| --- | --- |
| 09:00 | **Autobus 13/14/15 Sliema → Valletta** (~20–25 min) — **karta** |
| 09:30 | **Prom Grand Harbour → Birgu/Vittoriosa** (~10 min, **~3 € one way / ~5 € return** osobno, taryfa dzienna; po ~19:30 nocna **3,50 € one way / 5,20 € return**). **Barrakka Lift** — winda kosztuje **1 € za bilet powrotny tego samego dnia (w górę i w dół)**; z biletem **return promu winda jest w cenie** (osobno płacisz tylko, gdy prom bierzesz w jedną stronę). 💡 Posiadacze **personalizowanej karty Tallinja** mają ten prom i windę za darmo — Explore (na okaziciela) raczej nie, zapytaj przy zakupie. |
| 10:00–11:30 | ⭐ **Fort St Angelo** (10 €) — ⚠️ średnio warta; historia + widoki |
| 11:30–12:30 | ◽ **Pałac Inkwizytora** 8 € ⚠️ · **Maritime Museum** 8 € ⚠️; max 1 |
| 12:30–13:30 | Lunch Birgu |
| 14:00–15:00 | ⭐ **Senglea – Gardjola Gardens** — **bezpłatnie**, jeden z najlepszych widoków na Vallettę. 🕐 spokojne popołudnie |
| 15:30–17:00 | ✅ **Darmowy widok z Barrakki** albo tania **dgħajsa (łódka)** przez port (~3–5 €) — klimat i widoki z wody · *(pełniejsza alternatywa: **Rejs po Grand Harbour** 1,5 h, 15–25 €)* |
| 17:30 | **Autobus 13/14/15 → Sliema** — karta |

> 🎉 ⚠️ **Alternatywy:** zamiast Fort St Angelo (12 €) — darmowy spacer nabrzeżem Birgu + widoki z Gardjola; zamiast drogiego rejsu (15–25 €) — tania **dgħajsa (łódka)** przez port ~3–5 € albo darmowy widok z Barrakka; tani „widok z wody" — prom Grand Harbour w obie strony (~5 € return). Jeśli wybierzesz **dgħajsę**, rozważ też pełniejszą opcję — dłuższy **Rejs po Grand Harbour** (1,5 h, 15–25 €). **Uwaga:** dgħajsa to atrakcja, nie pewny środek transportu — jako plan awaryjny na powrót przez port używaj zwykłego promu Valletta–Three Cities (~3 € one way, pewny rozkład).
>

### Dzień 5 · sob 03.10 — Comino + Blue Lagoon 222 + prom Ċirkewwa→Comino 🟢

> ✅ Dzień na wodzie, mało logistyki — dobra odskocznia po dwóch intensywnych dniach (Gozo D3, miasta D4). Dla wczesnego wejścia do Blue Lagoon wybierz **bezpośredni prom z Ċirkewwy/Marfy**, a nie rejs ze Sliemy — taniej (~15 € return/os) i szybszy dojazd. Wieczorem opcjonalna **Notte Bianca** w Valletcie.
>

| Godz. | Co |
| --- | --- |
| 07:15 | **Autobus 222** Sliema → **Ċirkewwa** (~75–110 min) — karta |
| ~09:00 | ⭐ **Prom Comino Ferries** Ċirkewwa → Blue Lagoon (15 € return/os; online bywa ~14 €). 🕐 pierwszy kurs = mniej tłoku. *Bilet elastyczny (bez godziny) — kup online i przyjedź rano na wybrany kurs.* |
| 09:45–13:00 | ⭐ **Blue Lagoon** — kąpiel (woda ~22–23 °C); ◽ **Crystal Lagoon** / jaskinie |
| 13:00–14:00 | ◽ Spacer na **Wieżę św. Marii** (~30 min w jedną stronę) — widoki na wyspę z góry. ⚠️ **Nie gwarantowana**: dostępność zależy od otwarcia przez Din l-Art Ħelwa, pogody i obecności obsługi — jeśli zamknięta, zostań w Santa Maria Bay |
| 14:00–15:00 | **Prom powrotny** → Ċirkewwa |
| 15:00–16:30 | **Autobus 222** → Sliema (~75–110 min) — karta |

> 🎉 **⚠️ Landing pass na Blue Lagoon (bezpłatny, osobny od biletu na łódź):** jeśli planujesz **zejść na ląd** przy Blue Lagoon, najpierw zarezerwuj **darmowy poranny slot AMS** (08:00–13:00; okna: 08:00–13:00 · 13:30–17:30 · 18:00–22:00) na **bluelagooncomino.mt / blcomino.com**, a przy zakupie Comino Ferries sprawdź, czy potwierdzenie zawiera dostęp/QR. Jeśli nie — pobierz bezpłatny pass z AMS. Jeśli pływasz **z pokładu** (bez schodzenia do kontrolowanego obszaru) — pass niepotrzebny.
>

> Czy warte w październiku: ✅ tak przy wczesnym starcie. 🕐 wypłyń przed 09:00. **Nie rób pełnego obchodu wyspy** — wybierz Crystal Lagoon **albo** wieżę św. Marii + Santa Maria Bay (spokojniejsza plaża); wieża św. Marii to opcja zależna od otwarcia. **Rejs ze Sliemy to osobna, droższa opcja** (ok. 28–40 €): sprawdź konkretną ofertę, bo nie każdy rejs obejmuje właściwy slot landing passu (czasem wymaga popołudniowego 13:30–17:30).
>

> 🎉 **Opcjonalny wieczór D5 — Notte Bianca w Valletcie (sob 3.10):** darmowy festiwal sztuki — pałace, muzea i ogrody Valletty otwarte do późna (~19:00–4:00). Bonus po dniu na Comino. Jeśli się wybierzesz, wróć ok. **~21:30**, bo D6 (Blue Grotto rano) zaczyna się wcześnie.
>

### Dzień 6 · nd 04.10 — Blue Grotto (rano) + Marsaxlokk (obiad/klimat) 13/74/81/119/201 · karta 🕐

> ✅ Kolejność: **Blue Grotto NAJPIERW** — rano najlepsze światło (blask w jaskiniach do ~13:00) i najmniej ludzi. Potem Marsaxlokk na obiad/klimat (łódki luzzu, promenada, port). Niedzielny targ rybny jest **bonusem „best-effort"** — i tak zwykle zniknie do ~12:30, gdy my jesteśmy przy Blue Grotto, więc nie planujemy wokół niego.
>

| Godz. | Co |
| --- | --- |
| 07:45 | **Autobus 13/14/15** Sliema → Valletta (~20 min) — karta |
| 08:35 | **Autobus 74** Valletta (stanowisko A5) → **Blue Grotto / Wied iż-Żurrieq** (wysiądź **Panorama**, ~42 min; niedziela co 60 min, bądź przed 08:30) — karta |
| 09:15 | ⭐ **Rejs łódką po Blue Grotto** (~10 €/os, ok. 20–25 min; kupno na miejscu, warto mieć gotówkę) — **🕐 najwcześniej, przed grupami; najlepsze światło**. ⚠️ Zależy od pogody/stanu morza — przy wzburzeniu rejsy mogą być wstrzymane |
| 09:45–10:15 | ⭐ **Klifowy punkt widokowy** — gratis, widok na łuk i Filflę |
| 10:15–11:30 | ◽ **Ħaġar Qim + Mnajdra** (10 €, UNESCO) — ~15–20 min pieszo — ⭐ warte (klify) |
| 11:45 | **Autobus → Marsaxlokk**: 201 → lotnisko → 119 **lub** 74 → Valletta → 81/85 — karta |
| 12:45 | ⭐ **Marsaxlokk + obiad z owoców morza** (promenada, łodzie luzzu, stoiska). ⚠️ Uczciwie: dojście ~12:45 oznacza, że **rybna część targu zwykle już zniknęła** (najbardziej lokalne stragany kończą ok. południa) — zostają pamiątki, produkty i klimat portu. To świadomy kompromis przy priorytecie "Blue Grotto najpierw". |
| 15:30–16:30 | **81/85 → Valletta → 13/14/15 → Sliema** — karta |

> 🎉 ⚠️ **Dlaczego Blue Grotto najpierw:** jaskinie mają najlepsze światło i najmniej ludzi rano (do ~13:00). Marsaxlokk zostaje na obiad/popołudnie — klimat portu i luzzu są cały dzień, a targ rybny (jeśli trafisz w niedzielę) i tak zwykle kończy się przed południem. **Świadomie rezygnujemy z porannego wyboru straganów** na rzecz światła w jaskiniach. Nie dzielimy tego na 2 dni: oba są na południu, każda przesiadka to ~1–1,5 h bez bezpośredniego połączenia.
>

### Dzień 7 · pon 05.10 — Mdina + Rabat → zachód słońca NAD MORZEM na klifach Dingli 202 + 201 · karta 🕐

| Godz. | Co |
| --- | --- |
| 08:45 | Pieszo na przystanek **Ferries (Sliema)** |
| 09:00 | **🕐 Autobus 202 → Mdina/Rabat** (co ~60 min, ~53 min) — karta |
| 10:00–12:30 | ⭐ **Mdina** — spacer po Cichym Mieście + **bastiony** (bezpłatnie). 🕐 przed wycieczkami (~10:00). ◽ Katedra+Muzeum — combo **15 €** (ulg. 10 €) ⚠️ pominąć |
| 12:30–13:30 | Lunch **Fontanella Tea Garden** (widok, ciasta) |
| 13:45–15:00 | ⭐ **Rabat** (pieszo ~10 min) — spokojniejszy, mieszkalny klimat. ◽ **Katakumby św. Pawła** 10 € ⚠️ · **Domus Romana** 6 € ⚠️ |
| 15:15–15:45 | **Autobus 201 → Dingli** (~20 min) — karta |
| 15:45–18:20 | 🌅 **Klify Dingli + Miġra l-Ferħa** — spacer wzdłuż zachodnich klifów. Zachód 5.10 wypada **ok. 18:40** — bądź na miejscu na ~17:30, zostań do ~18:45 |
| 18:45–19:15 | **Autobus 201 → Mdina/Rabat** (sprawdź ostatni kurs **tego samego dnia**) |
| 19:30–20:15 | 🌙 **Wieczorna Mdina** — podświetlone mury, prawie puste uliczki |
| 20:30 | **Autobus 202 → Sliema** (ostatni ~21–22:00 — nie przegap) |

> 💡 **Dlaczego zachód na Dingli, a nie z bastionów Mdiny?** Z bastionów Mdiny słońce zachodzi **nad lądem** (płaskowyż na zachód), a z **klifów Dingli nad otwartym morzem** — to ikoniczne ujęcie Malty. Zostajemy więc w okolicy Mdiny cały dzień (jeden powrót, bez podwójnego dojazdu): Rabat → Dingli na zachód → wieczorna (podświetlona, pusta) Mdina → 202 do Sliemy.
>

> ⚠️ **Zabezpieczenie:** autobusy **201 z Dingli wieczorem są rzadkie (~co 60 min)** — sprawdź w aplikacji Tallinja **ostatni kurs powrotny tego dnia** (na 5.10). Jeśli ostatni jest zbyt wcześnie na zachód (~18:40) — oglądaj zachód z **bastionów Mdiny** (bezpieczna opcja, nad doliną, nie nad morzem) i traktuj Dingli jako popołudniowy widok. Na powrót z Mdiny **wsiadaj na pierwszym przystanku** (pełny autobus nie zatrzyma się na kolejnym). ◽ **Rotunda w Moście** (darmowa kopuła): gdybyś jechał inną trasą przez Vallettę (51/52/53), możesz wysiąść w Moście na ~20 min, potem dalej do Mdiny.
>

### Dzień 8 · wt 06.10 — Wylot 13/14/15 + 88

| Godz. | Co |
| --- | --- |
| 08:30–08:45 | Wyjazd z hotelu: **13/14/15 → Valletta** (karta) → **88 → lotnisko** (karta). Trasa przez Vallettę ~1 h; 88 z Valletty co ~15–30 min — **nie trzeba startować o 07:30**, jest spokojny poranek (śniadanie, spakowanie) |
| 10:00–10:15 | Na lotnisku (**~2 h 15 min przed odlotem 12:30** — tyle zaleca Malta Airport dla lotu w **Schengen**; 3 h to tylko non-Schengen/long-haul). **Karta nadal ważna** (wygasa ~06.10 ok. 13:00); awaryjnie miej ~2,50 € na bilety. Opcja szybsza/direct: **nie planuj na tym powrotu** — część źródeł wskazuje, że X2 przeniesiono na linie **TD2/TD3 (poza Explore)**; pewny wariant to **13/14/15 + 88** (karta) |

## 5. Oceny atrakcji — co warte, co opcjonalne

Legenda: ⭐ = warta (polecam) · ✅ = warta · ⚠️ = średnio/opcjonalnie · 🚫 = można pominąć.

### Valletta (D2)

| Atrakcja | Cena | Ocena |
| --- | --- | --- |
| **St John's Co-Cathedral** (Caravaggio, audio-guide) | 15 € | ⭐ WARTE — barok na miarę Rzymu, najlepsze wnętrze Malty |
| **Upper Barrakka Gardens + salut 12:00** | bezpłatnie | ⭐ WARTE — najlepszy widok na Grand Harbour |
| **Lower Barrakka Gardens + mury** | bezpłatnie | ✅ warte |
| **Grandmaster's Palace** | 12 € | ⚠️ Sala Tronowa zamknięta; State Rooms tylko w wybrane dni od 13:30 — pomiń |
| **Saluting Battery** (od środka) | ~3–5 € | ⚠️ opcjonalnie — salut i tak widzisz z Barrakka za darmo |
| **MUŻA** (sztuka) | 8 € | ⚠️ opcjonalnie, jeśli lubisz sztukę |
| **National Museum of Archaeology** | 10 € | ⚠️ opcjonalnie, historia |
| **Malta Experience** (show) | 16 € | ⚠️ opcjonalnie |
| **Malta 5D** | 15 € | 🚫 można pominąć (show, nie historia) |
| **Lascaris War Rooms** | ~14 € | ⚠️ opcjonalnie, dla fanów II wś. |
| **Casa Rocca Piccola** | 10 € | ⚠️ opcjonalnie, pałac ze schronami bombowymi |

### Gozo (D3)

| Atrakcja | Cena | Ocena |
| --- | --- | --- |
| **Victoria / Citadella** | bezpłatnie | ⭐ WARTE — panorama i mury |
| **Świątynie Ġgantija / Ta' Kola combo** | 10 € | ⭐ WARTE — starsze niż piramidy, UNESCO |
| **Bazylika Ta' Pinu** | bezpłatnie | ✅ warte, klimatyczne |
| **Pola solne** | bezpłatnie | ✅ opcjonalnie, ciekawe |
| **Ramla Bay / Xlendi Bay (plaża)** | bezpłatnie | ✅ opcjonalnie, jeśli czas na kąpiel |

### Three Cities (D4)

| Atrakcja | Cena | Ocena |
| --- | --- | --- |
| **Gardjola Gardens (Senglea)** | bezpłatnie | ⭐ WARTE — jeden z najlepszych widoków na Vallettę |
| **Fort St Angelo** | 10 € | ⚠️ średnio — widoki i historia, ale część uroku widzisz za darmo |
| **Pałac Inkwizytora** | 8 € | ⚠️ opcjonalnie, historia |
| **Maritime Museum** | 8 € | ⚠️ opcjonalnie, morska historia |
| **Darmowy widok z Barrakki** | bezpłatnie | ✅ WARTE — najlepszy widok na port za darmo |
| **Tradycyjna dgħajsa (łódka)** przez port | ~3–5 € | ✅ rekomendacja — tani klimat i widoki z wody |
| **Rejs po Grand Harbour** | 15–25 € | ◽ alternatywa — tylko jeśli chcesz dłuższy, pełniejszy rejs |

### Comino (D5)

| Atrakcja | Cena | Ocena |
| --- | --- | --- |
| **Blue Lagoon + prom z Ċirkewwy** | ~15 € return | ⭐ WARTE przy wczesnym starcie (październik OK); rejs ze Sliemy to droższa alternatywa (28–40 €) |
| **Crystal Lagoon** (kąpiel, mniej tłoku) | w cenie rejsu | ✅ warte, spokojniejsza |
| **Jaskinie morskie / Caves** | w cenie rejsu | ✅ warte |

### Marsaxlokk + Blue Grotto (D6)

| Atrakcja | Cena | Ocena |
| --- | --- | --- |
| **Niedzielny targ rybny** | bezpłatnie | ⭐ WARTE — najbardziej „malteski" widok |
| **Łodzie luzzu + port** | bezpłatnie | ⭐ WARTE |
| **Blue Grotto — rejs łódką** | ~10 € | ⭐ WARTE, ale **tylko rano** (przed ~13:00); po południu światło i tłumy gorsze. ⚠️ Zależny od pogody/morza |
| **Blue Grotto — punkt widokowy z klifu** | bezpłatnie | ✅ warte, nawet bez rejsu |
| **Ħaġar Qim + Mnajdra** (UNESCO) | 10 € | ⭐ warte dla lubiących prehistorię; spektakularna lokalizacja nad klifem |
| **St Peter's Pool** (kąpielisko) | bezpłatnie | ✅ opcjonalnie, ładna zatoka (dojście ~20–30 min pieszo) |

### Mdina + Rabat (D7)

| Atrakcja | Cena | Ocena |
| --- | --- | --- |
| **Mdina — spacer + bastiony** | bezpłatnie | ⭐ WARTE — największy „must" całego dnia |
| **Brama Mdiny / uliczki / widoki** | bezpłatnie | ⭐ WARTE |
| **Katedra św. Pawła + Muzeum Katedralne** | 15 € (combo; ulg. 10 €) | ⚠️ można pominąć — skromniejsze niż St John's |
| **Muzeum Historii Naturalnej** | 7 € | ⚠️ opcjonalnie |
| **Katakumby św. Pawła (Rabat)** | 6 € | ⚠️ tylko jeśli lubisz katakumby |
| **Domus Romana (Rabat)** | 6 € | ⚠️ opcjonalnie, rzymskie mozaiki |
| **Klify Dingli (zachód słońca)** | bezpłatnie | ✅ warte widoku, ale uwaga na rzadkie autobusy wieczorem |

> 🏆 **Wniosek:** płatne „pewniaki" to **St John's 15 + Ġgantija 10 + Ħaġar Qim 10 + Blue Grotto ~10 + Fort St Angelo 10 = ~55 €** (Fort i Ħaġar Qim ⚠️ opcjonalne). Cała reszta płatna to opcjonalne, a większość najpiękniejszych rzeczy jest darmowa. **Nie kupuj biletów z góry — wybieraj wg tabeli na miejscu.**
>

> 💡 **Heritage Malta Multisite Pass kosztuje 30–60 € (warianty ulgowe)** — **nie opłaca się automatycznie**. Kup go tylko, gdy suma konkretnych biletów z planu przekracza 60 €. Przy obecnym zestawie D3+D4+D6+D7 ≈ **44 €** (Ġgantija 10 + Fort St Angelo 10 + Ħaġar Qim 10 + Rabat Combo 14; bez Ħaġar Qim ≈ 34 €) — lepiej kupować bilety pojedynczo lub Rabat Combo.
>

## 5a. Gdzie zjeść — smacznie i w rozsądnych cenach (wg dnia)

Ceny orientacyjne: przekąska **€0,50–8** · prosty lunch **ok. €10–18** · lokal średniej klasy **ok. €20–35** · lokal premium **od ok. €45/os**. Same symbole $/$$/$$$ nie są pewnym dowodem ceny — licz z menu.

| Dzień / miejsce | 🍴 Rekomendacje (główna opcja · tańsza opcja / przekąska) |
| --- | --- |
| **D1 · St Julian's / Spinola** | **Sticky Fingers BBQ** — pewna opcja wieczorna (ok. €9–21/os., pełne zamówienie może wyjść wyżej; ok. 17:00–22:30, można rezerwować) · **Tony's Bar** — lokalny bar + szybkie jedzenie przy Spinola Bay (ok. €5–10/os., przekąska / prosty posiłek; ok. 6:30–23:57, bez rezerwacji). **Two Buoys** tylko jako brunch / lunch (do ok. 16:00), nie kolacja. **Ta' Kris / Wigi's / Zeri's** — płatne alternatywy wyższej półki (Wigi's ~€20–50/os., Zeri's ~€20–35/danie), nie opcje „na tanio". ⚠️ **Singapore Noodle House pomiń** — zamknięte we wtorki |
| **D2 · Valletta lunch** | **Ta' Nenu** — lokalnie (ftajjar €13,50; królik ~€10,50–20,50; wieczorem potwierdź stolik) · **pastizzeria / pastizzi** jako przekąska · **Café Jubilee** — casual lunch, ale nie „bardzo tanio" (ok. €10–30/os.) · **Aaron's Kitchen** — dobre lokalne bistro, średnia półka, rezerwacja · **Rubino** — premium (Bib Gourmand), nie jako tania opcja · **Submarine** — tylko po sprawdzeniu tego samego dnia (jeden listing = czasowo zamknięty). Unikaj turystycznego deptaka na lunch |
| **D3 · Gozo (Victoria)** | **Ta' Rikardu** przy Cytadeli — dobry lunch / wczesny posiłek (sery, ftira Ghawdxija; potwierdź godziny, ok. 11:00–17:00/19:00) · **Coffee Break Café & Bistro** — najbezpieczniejszy wybór (ok. €3–15/os., blisko dworca autobusowego) · **McSims** pastizzi — tańsza przekąska · **Ta' Ċetta** — średnia półka (pt ok. 8:00–22:00). ⚠️ **Café San Martino** słabo oceniane (nie główna rekomendacja); **Kafe+** pomiń (brak aktualnych godzin) |
| **D4 · Birgu (Three Cities)** | **BeBirgu** — najbardziej praktyczna pełna opcja (ok. 7:00–23:00, pizza ~€13–17) · **Il-Girbi** — ftira €7,50 / pasta €12–14 / królik €17 (czw ~7:30–20:00) · **Tal-Petut** — tylko wariant premium (18:30–22:00, ok. €45–50/os., rezerwacja zdecydowanie wskazana). ⚠️ **Harbour Club jest w Vallettcie, nie w Birgu** — usunięty; **Café Riche** zamknięty w czwartek — pomiń. *(Plaża → kioski / pastizzi)* |
| **D5 · Comino** | Brak porządnych restauracji — **główny plan to prowiant zabrany przed przeprawą** (ftira, kanapki, owoce, woda w St Julian's / po drodze). Kiosku na wyspie nie traktuj jako pewnego taniego obiadu |
| **D6 · Marsaxlokk** | **Liska** — praktyczny lunch (nd ok. 11:30–16:00; średnia półka; wybór ze względu na godziny i wykonalność planu, nie „najświeższa ryba") · tańsza opcja: **własna ftira / pastizzi / kanapki** (targ to głównie surowe ryby — nie licz na gotowe tanie danie) · **Tartarun** — tylko wariant premium (nd ~12:00–15:00, rezerwacja) · **Ir-Rizzu** — plan B, ale godziny niespójne. Unikaj głównego nabrzeża w niedzielę (drożej); pytaj „co dziś złowiono" |
| **D7 · Mdina / Rabat** | **Ta' Doni** (Rabat) — lunch / wczesny posiłek: kanapki, platters, sałatki, ok. €5–25 (ok. 9:00–22:00) · **Is-Serkin / Crystal Palace** — pastizzi €0,50–0,60 (przekąska, nie pełny obiad) · **Fontanella** — jako opcja widokowa / deserowa (ok. €9–22/os., nie „na tanio") · **Don Mesquita** — lunch, nie posiłek po zachodzie (zamyka ~18:00) · **Il-Baxa** — lunch w Rabacie (pon ~11:00–17:00, nie po Dingli) · **Il-Mahzen** — dzienny przystanek na przekąski / napoje (ok. 9:00–17:30), nie pełna kolacja · **La Piazza** — tylko awaryjnie po potwierdzeniu (niespójne adresy) · **Coogi's / Trattoria AD 1530** — średnia półka, nie pewna późna kolacja |

## 6. Budżet (na osobę)

| Pozycja | Kwota |
| --- | --- |
| **7-dniowy Explore (autobusy D1–D8)** | 25 € |
| Prom Valletta→Three Cities (D4) | ~5 € (return) |
| Barrakka Lift (D3, zejście na Lascaris; w D4 gratis z return promu) | 1 € |
| Szybki prom Valletta→Gozo (D3, ~7,50 € w jedną) | ~15 € return |
| Comino prom Ċirkewwa→Comino (D5) | ~15 € return |
| Blue Grotto rejs (D6) | ~10 € (bufor do 12) |
| Wejścia „pewniaki" (St John's 15 + Heritage ≈44, z Ħaġar Qim) | ~59 € |
| **RAZEM (domyślny wariant: Valletta→Gozo, z Ħaġar Qim)** | ~130 € |
| − budżetowo: zamiast szybkiego promu — Gozo Channel (222 + 4,65 €, bez windy) | −~11 € → ~119 € |
| + jeśli na Comino wybierzesz rejs ze Sliemy zamiast promu z Ċirkewwy | +~10–20 € |

> **Budżet biletów i transportu** wynosi orientacyjnie **~130 € na osobę** przy szybkim Gozo Highspeed (z Ħaġar Qim 10 €) albo **~119 €** przy wariancie Gozo Channel. Kwoty zakładają osobę dorosłą, bilet VFS return, St John's (15 €), zestawy Heritage Malta (≈44 €: Fort St Angelo 10 + Ġgantija/Ta' Kola 10 + Ħaġar Qim 10 + Rabat Combo 14) i orientacyjny rejs Blue Grotto (10 €). Nie obejmują jedzenia, noclegu, lotów ani zakupów. Do wariantu online dolicz możliwą **booking fee St John's** oraz niewielki bufor na **Airport Direct** (3–3,50 €) i zmienną cenę Blue Grotto.
>

## 7. Praktyczne

- **7-dniowy Explore (25 €)** pokrywa **wszystkie autobusy D1–D8** — także dojazd do Valletty w D2/D3 (13/14/15 zamiast promu).
- Karta zaczyna liczyć **od pierwszego użycia** → kup na lotnisku, przyłóż pierwszy raz na **88** w D1. Na lotnisko: 88 (w karcie) lub TD2 (+3 €).
- **Osobno płacisz:** szybki prom Gozo (D3, ~15 € w obie przez Valletta; przez Sliema ~17 €), prom Valletta→Three Cities (D4, ~5 € return), prom Comino z Ċirkewwy (D5, ~15 €), rejs Blue Grotto (D6, ~10 €), wstępy.
- **Autobusy na Gozo (301/307/308), Blue Grotto (74/201), 119 i Mdina (202) jeżdżą co ~60 min** — trzymaj się rozkładu w aplikacji **Tallinja**.
- **🕐 kluczowe godziny unikania tłumów:** St John's 09:00 · Barrakka przed 12:00 · Comino przed 09:00 · Mdina ~10:00 · **Blue Grotto ~09:15** · Marsaxlokk na obiad.
- **Październik:** lekka kurtka wieczorem, kapcie do wody, sprawdzaj prognozę przed rejsami.
- **St John's:** czynne 09:00–16:45, ostatnie wejście ~16:00.

> 🎉 **Wydarzenia w terminach wyjazdu (uwaga na tłok/hałas):** **Notte Bianca w Valletcie** — **sob 3.10** (wieczór D5), cała Valletta otwarta za darmo do późna (opcjonalny wieczorny bonus, wróć ~21:30, bo D6 zaczyna się wcześnie). **Defected Malta 1–4.10** (UNO, Fort St Elmo, Café del Mar) — jeśli nie lubisz dużych imprez, licz się ze wzmożonym ruchem w St Julian's; w Argento poproś o pokój od cichej strony.
>

## 8. Rekomendacje i ostrzeżenia z polskich blogów

Zebrane z blogów (m.in. zLukaszem, podrozepoeuropie, proste-podroze, podrozebezosci, zyciewpodrozy, wheretravelmarta, travelfairy, pioremprzezswiat).

### Godziny / kolejki (najczęściej powtarzane)

- **St John's Co-Cathedral** — blogi polecają rezerwację online w sezonie, by ominąć kolejkę. Potwierdza „bądź na otwarciu 09:00".
- **Blue Lagoon (Comino)** — „przypłyń pierwszym promem przed 09:00"; „w środku dnia w sezonie robi się naprawdę ciasno".
- **Niedzielny targ w Marsaxlokk** — „najlepiej przed 9:00, zanim zjadą autokary". ⚠️ Konflikt z Blue Grotto rozstrzygnięty na korzyść Blue Grotto rano — patrz D6.
- **Mdina** — „wróć po zachodzie słońca, kiedy wycieczkowicze wyjechali — podświetlone mury i prawie puste uliczki".
- **Blue Grotto** — „rejsy to jedne z najpopularniejszych atrakcji, w sezonie długie kolejki" → potwierdza poranny dojazd ~09:15.

### Co polecają DOŁOŻYĆ

- **Mdina + Rabat razem**: „Rabat ma mniej turystów i bardziej mieszkalny klimat"; obok murów **Domvs Romana** (mozaiki) — opcjonalne 6 € w D7.
- **Għajn Tuffieha / Golden Bay** zamiast zatłoczonej Mellieha Bay, jeśli chcesz plażę.
- Na Gozo: **jaskinia Tal-Mixta** z widokiem na pomarańczową plażę **Ramla** oraz **Victoria/cytadela** — „najlepszy wybór pamiątek na wyspie".
- **Klify Miġra l-Ferħa** (obok Dingli) — szlaki widokowe na szczycie klifów (opcja w D7).
- **Combo ticket Malta Heritage** (Heritage Malta) — wstęp do 4 obiektów, ważny 30 dni. Przy obecnym planie (≈44 €) niekoniecznie; zamiast tego **Rabat Combo** (katakumby+Domvs, 14 € dla dorosłego).
- **Rotunda w Moście** (darmowa kopuła) — szybki, tani przystanek po drodze (opcja w D7).
- Na Comino: **spacer do Wieży św. Marii** (~30 min w jedną stronę) — widoki z góry (opcja w D5).

### Ostrzeżenia / co odradzają

- **Autobusy potrafią się spóźniać i bywają pełne** — „nie polegaj na rozkładzie"; „wracając z Mdiny stój na pierwszym przystanku, bo pełny autobus nie zatrzyma się na kolejnym".
- **W miastach straszne korki** — uwzględnij opóźnienia; autobus z lotniska potrafił przyjechać ±30 min.
- **Kierowca nie zawsze ma resztę** — płać kartą zbliżeniową lub miej odliczone.
- **Explore Card nie działa na linie TD** (jest ich niewiele; do wszystkiego w planie dojedziesz bez nich).
- **Woda z kranu** — „zdatna, ale słony smak"; lepiej butelkowana.
- **Gniazdka typu G (brytyjskie)** — zabierz przejściówkę (to kluczowe!).
- **Plaże piaszczyste są nieliczne** — na Malcie Golden Bay, na Gozo Ramla Bay; reszta to klify/skaliste kąpieliska.
- **Gozo warto dać dodatkowy czas** — część blogerów sugeruje nocleg na Gozo; u nas jeden intensywny dzień wystarczy, ale bez ściemy: nie zdążysz „wszystkiego".
- **Bilety do wnętrz i rejsy** — St John's i Ġgantija to „pewniaki", a muzea/drogie show (Malta Experience) można pominąć bez żalu.

## 9. Co zarezerwować wcześniej online (z linkami)

Sprawdzone na 2026. Niektóre rzeczy **trzeba** zarezerwować, inne **warto** (kolejki).

### 🔴 Zarezerwuj wcześniej online

### 1. Comino / Blue Lagoon — bezpośredni prom z Ċirkewwy/Marfy + darmowy landing pass

- **Najlepsza opcja dla planu D5:** bezpośredni prom z **Ċirkewwy/Marfy** (Comino Ferries) — **15 € return/os**. Online bywa ~14 € (oszczędzasz ~1 €), ale **w budżecie przyjmij 15 €**.
- ✅ Bilet jest **ważny 3 mies. i elastyczny** — bez przypisanej godziny/dnia; wchodzisz na pokład w **dowolnym punkcie odjazdu** (Marfa ~08:30 / Ċirkewwa ~08:40 pierwsze kursy). ⚠️ Nie rezerwuje konkretnego odejścia — kup online, ale **przyjedź odpowiednio wcześnie na wybrany kurs** (uwzględnij kolejkę).
- **Landing pass:** jeśli planujesz **zejść na ląd** przy Blue Lagoon, najpierw zarezerwuj **darmowy poranny slot AMS** (**08:00–13:00**, formularz pokazuje „Morning"), a przy zakupie Comino Ferries sprawdź, czy potwierdzenie rzeczywiście zawiera dostęp/QR. Jeśli nie — pobierz bezpłatny pass z AMS → https://blcomino.com/product/blue-lagoon/ · bilety: https://www.cominoferries.com/buy-tickets-ferry/. ⚠️ Comino Ferries obecnie deklaruje, że ich boarding pass obejmuje wejście i nie wymaga dodatkowego QR — ale **nie rozstrzygaj tego bezwarunkowo**; zweryfikuj po zakupie. Jeśli zostajesz na łodzi i nie schodzisz do kontrolowanego obszaru — pass niepotrzebny.
- **Rejs ze Sliemy to osobna, droższa opcja** (ok. 28–40 €): sprawdź konkretną ofertę, bo ceny, godziny, długość postoju i slot landing passu zależą od wybranego rejsu (niektóre wymagają popołudniowego 13:30–17:30). Nie wybieraj ogólnej kategorii GetYourGuide ze Sliemy — to nie konkretna oferta.
- **Werdykt dla D5:** dla zwykłego transportu na Comino domyślnym wariantem jest Comino Ferries z **Ċirkewwy/Marfy** — **15 € return/os**. To zwykle **znacznie taniej** niż pełnodniowy rejs ze Sliemy, ale określenie „~2× taniej" jest prawdziwe tylko przy porównaniu z ofertą za ok. 30 € (przy rejsie za 40 € różnica jest większa). Comino Ferries **nie jest równoważnikiem rejsu widokowego** — sprawdź osobno, czy konkretna oferta obejmuje oglądanie jaskiń lub Crystal Lagoon (Crystal Lagoon to głównie atrakcja oglądana od strony wody; z lądu dojdziesz głównie do punktów widokowych). Droższy rejs ze Sliemy bierz **tylko**, jeśli (a) nie chcesz ogarniać landing passu (zostajesz na łodzi), (b) chcesz jaskinie/Crystal Lagoon z pokładu albo (c) wolisz wygodę bez logistyki dojazdu.

### 2. Szybki prom na Gozo — D3, domyślnie z Valletty (decyzja)

Gozo Highspeed to pasażerski prom (tylko pieszo, bez auta). **Domyślnie w D3 startujemy z Valletty** — z Sliemy jedziemy autobusem 13/14/15 na karcie (~25 min) na dworzec, a z terminala **Lascaris Wharf** (pod murami — zejście windą Barrakka 1 € / darmowym X133 / ~10 min pieszo) mamy **bezpośredni rejs ~45 min** — szybszy oraz tańszy niż trasa przez Buġibba (Comino w D5 zostaje przez Ċirkewwę).

- **Domyślne — Valletta → Gozo:** **~45 min bezpośrednio** do Mġarr, **~7,50 € w jedną (~15 € w obie)**, całoroczne. Dojazd z Sliemy na karcie (13/14/15, ~25 min). Kup bilet online.
- **Alternatywa — Sliema → Gozo (przez Buġibba):** ~1 h 15 min (krótka przesiadka), od **~8,50 €** one way (~17 € w obie), sezon marzec–październik — wygodniejsza, gdy chcesz wrócić prosto do St Julian's (ostatni kurs Mġarr→Sliema jest później: 21:15 → 22:30).

**Rozkłady na D3 (czw 1.10):**

- **Valletta → Gozo:** co ~godzinę; poranne **08:45 → 09:30**, **09:45 → 10:30**; ostatnie **20:45 → 21:30** i **22:15 → 23:00** (kurs 22:15 tylko pt–sob — w czw 1.10 go nie ma, ale my wracamy wcześniej).
- **Sliema → Gozo (przez Buġibba, alternatywa):** w sezonie; poranne odejścia ze Sliemy **06:45 · 08:45 · 09:45 · 11:45** → Mġarr ~1 h 15 min później (08:45 → ~10:00).

**Powroty z Mġarr na D3 (czw 1.10):**

- **→ Valletta:** co ~godzinę od **05:45 do 20:45** → Valletta ~45 min później; ostatni standardowy **20:45 → 21:30** i **22:15 → 23:00** (pt–sob). **Bierz kurs 16:45** → St Julian's ~18:00. **17:45 jest za późno** — → Valletta 18:30 → St Julian's ~19:15, poza oknem „~18:00–19:00".
- **→ Sliema (przez Buġibba, alternatywa):** odejścia **15:15 · 17:15 · 18:15 · 20:15 · 21:15**; **17:15 → ~18:30** prosto do Sliemy — osobny bilet ~8,50 €. Użyj, jeśli wolisz wracać do St Julian's bez przesiadki.

**Rezerwacja:** taryfy https://gozohighspeed.com/pages/fares · rozkład https://gozohighspeed.com/pages/schedule · bilet https://gozohighspeed.com/booking (lub ferryhopper.com). Na konkretny poranny kurs **kup bilet wcześniej online** — rezerwacja online zamyka się ~20 min przed rejsem, wejście na pokład i kasa zamykane 2 min przed odjazdem. Walk-on nie jest formalnie wykluczony (operator ma też kasę), ale przy kursie 08:45 nie warto ryzykować. ⚠️ Operator informuje o czasowych zmianach dojścia do terminala promowego w Mġarr (do 31.10) — dolicz kilka–kilkanaście minut na dojście wg oznaczeń na miejscu.

### 2a. Werdykt — dlaczego domyślnie z Valletty

- **Domyślne (Valletta → Gozo):** ~45 min bezpośrednio, **~7,50 € w jedną (~15 € w obie)**, dojazd busem 13/14/15 na karcie — **bez długiej 222 do Ċirkewwy**. Szybciej i taniej niż trasa przez Buġibba.
- **Alternatywa (Sliema → Gozo):** ~1 h 15 min (przez Buġibba), ~8,50 €/jedna strona (~17 € w obie) — tylko gdy zależy Ci na powrocie prosto do St Julian's (ostatni kurs 21:15 → 22:30).
- **Budżetowo (222 + Gozo Channel):** autobus 222 Sliema→Ċirkewwa + prom Gozo Channel ≈ 1h45–2h15, **~4,65 €** — tylko jeśli chcesz zaoszczędzić kosztem czasu i jazdy do Ċirkewwy.
- **Wniosek (decyzja):** jedziemy **szybkim promem z Valletty** — najkrótszy i najtańszy z szybkich wariantów, bez drugiej wyprawy do Ċirkewwy. **Comino (D5) zostaje przez Ċirkewwę** (prom 15 €), więc do Ċirkewwy jedziemy tylko raz.

### 🟡 Warto sprawdzić / rozważyć

### 3. St John's Co-Cathedral (Valletta) — bilet

- **Cena:** **15 €** dorosły (senior/student 12 €).
- **Godziny:** pon–sob **09:00–16:45**, ostatnie wejście **dokładnie o 16:00**.
- **Kup:** najlepiej przez **oficjalny checkout fundacji na ShowsHappening** → https://www.showshappening.com/st-johns-co-cathedral-foundation/st-john-s-co-cathedral-admissions-ticket?p=website (tam kieruje oficjalna strona katedry). Zapasowy link: https://www.stjohnscocathedral.com/buy-tickets/. ⚠️ Przy zakupie online może pojawić się dodatkowa **booking fee** — sprawdź kwotę końcową w checkoucie. Wariant awaryjny: wejście na miejscu od 09:00 — ale sprawdź aktualność godzin przed wyjazdem.
- ⚠️ **Ostrzeżenie:** *nie kupuj biletów od nieautoryzowanych pośredników*. **Nie traktuj GetYourGuide/Viator jako oficjalnego kanału sprzedaży samego biletu**; wycieczkę zewnętrzną bierz tylko, jeśli oferta wyraźnie potwierdza oficjalne wejście.

### 4. Heritage Malta Multisite Pass — 30–60 €, nie opłaca się przy obecnym zestawie

- **Ceny:** dorosły **60 €** · senior/student **45 €** · ważność **30 dni** od pierwszego użycia.
- Obejmuje m.in. Grand Master's Palace, Ċittadella Visitors' Centre i Malta National Aquarium.
- ⚠️ **NIE obejmuje** Ħal Saflieni Hypogeum ani Valletta Underground.
- **Kup:** https://heritagemalta.mt/store/c95/ (albo heritagemalta.mt → Store).
- **Werdykt dla planu:** **nie opłaca się** przy obecnym zestawie. Pojedyncze ceny: Fort St Angelo **10 €**, Ġgantija/Ta' Kola **10 €**, St Paul's Catacombs **6 €**, Domvs Romana **6 €**, Ħaġar Qim/Mnajdra **10 €**. D3+D4+D6+D7 ≈ **44 €** dla dorosłego (Fort St Angelo **10 €** + Ġgantija/Ta' Kola **10 €** + Ħaġar Qim **10 €** + Rabat Combo **14 €**; bez Ħaġar Qim ≈ 34 €) → kupuj bilety pojedynczo lub **Rabat Combo** (14 € dla dorosłego).

### 5. Blue Grotto — rejs łódką (kupno na miejscu)

- Tradycyjna łódź z **Wied iż-Żurrieq**, **kupno biletu na miejscu** — nie znaleziono oficjalnego systemu rezerwacji online.
- **Budżet orientacyjny:** przyjmij **10 € za osobę**, ale miej **bufor 10–12 €**; rejs ~**20–25 min**. Bilet kupuje się na miejscu w **Wied iż-Żurrieq** — brak potwierdzonego systemu rezerwacji online. Forma płatności bywa różna — miej gotówkę, kartę traktuj jako awaryjną.
- ⚠️ **Rejs zależy od pogody i stanu morza** — przy wzburzeniu może być wstrzymany. Bądź rano (~09:15), by uniknąć kolejki i mieć najlepsze światło.
- ✅ **Wariant awaryjny:** punkt widokowy Blue Grotto jest **bezpłatny** — jeśli łodzie nie kursują, zobacz widok z klifu i kontynuuj D6 zgodnie z planem (nie dziel go na 2 dni).

### 🟢 Kupujesz na miejscu / bez rezerwacji

- **Bilet autobusowy Explore 7 dni (25 €)** — Malta Public Transport Sales Office w Welcomer's Hall na lotnisku (oficjalne godziny ok. 08:00–19:30) albo automat biletowy / autoryzowane punkty (Agenda, 8TillLate, WH Smith). Link do sprzedaży: https://www.publictransport.com.mt/office-and-ticket-machine/ · https://www.publictransport.com.mt/fares-and-tickets/. ⚠️ Oficjalne strony MPT są niespójne co do Airport Direct/TD — dla bezpieczeństwa **nie zakładaj darmowego TD2/TD3/Airport Direct** Awaryjnie przyjmij **3 € za TD2/TD3** albo **3,50 € za TD1** i potwierdź zasady przy zakupie karty.
- **Szybki prom Gozo Highspeed** (Valletta→Mġarr, ~7,50 € w jedną (~15 € w obie); alternatywa przez Sliema ~8,50 €) — kup bilet online (patrz §2). Budżetowo zamiast tego: prom Gozo Channel (Ċirkewwa→Mġarr, 4,65 €, bilet pasażera powrotny, bez rezerwacji, first-come-first-served).
- **Prom Valletta→Three Cities** — ~3 € one way / ~5 € return (poza zwykłym Explore), na miejscu.
- **Mdina, Rabat, Fort St Angelo, Pałac Inkwizytora** — standardowe wejścia zwykle kupisz na miejscu, ale część obiektów ma też oficjalną sprzedaż online; przed wizytą sprawdź godziny otwarcia, ostatnie wejście i ewentualne zamknięcia. Nie zakładaj wyłącznie gotówki.

### 📋 Lista kontrolna przed wyjazdem

| Zadanie | Status |
| --- | --- |
| Comino — kup elastyczny bilet Comino Ferries (~15 € return, ważny 3 mies., dowolny punkt odjazdu) i **przyjedź rano**; sprawdź, czy potwierdzenie zawiera wejście na Blue Lagoon, jak nie — pobierz osobny darmowy pass (slot poranny) | ⬜ |
| Gozo — domyślnie szybki prom z Valletty (Gozo Highspeed, ~7,50 € w jedną (~15 € w obie); alternatywa przez Sliema ~8,50 €) — dojazd busem 13/14/15, kup bilet online na kurs 08:45. Budżetowo zamiast tego: 222 + Gozo Channel (~4,65 € w obie) | ⬜ |
| St John's Co-Cathedral — kup bilet przez oficjalny link fundacji (showsHappening) https://www.stjohnscocathedral.com/buy-tickets/ | ⬜ |
| Heritage Malta Multisite Pass (30–60 €) — tylko jeśli suma konkretnych biletów wynosi co najmniej 60 €; przy obecnym zestawie ≈44 € dla dorosłego (z Ħaġar Qim) kupuj pojedynczo/Rabat Combo (nie ma automatu „od 4 obiektów" — liczy się realna suma) | ⬜ |
| Przejściówka do gniazdek (typ G) — nie zapomnij! | ⬜ |
| Tallinja Explore 7 dni (25 €) — kup na lotnisku (Welcomer's Hall, ~08:00–19:30) przy przylocie | ⬜ |

Plan: 29.09–06.10 · Argento (St Julian's) · 7-dniowy Explore 25 € · Gozo D3 (szybki prom z Valletty), Comino D5 (prom z Ċirkewwy), Mdina D7. Budżet ~130 €/os. przy szybkim Gozo albo ~119 €/os. przy wariancie Gozo Channel — **orientacyjnie transport + bilety wstępu (dorosły), bez jedzenia, noclegu, lotów i zakupów**. Do wariantu online dolicz możliwą booking fee St John's oraz bufor na Airport Direct (3–3,50 €).

